...
const Conditional = ({ condition = true, children }) =>
  condition ? <Info>{children}</Info> : null;
...
const Duration = ({ data }) => {
  ...
  return (
    <LightText>{new Date(data * 1000).toISOString().substr(11, 8)}</LightText>
  );
};
  ...
  componentWillReceiveProps(nextProps) {
    if (this.props.task !== nextProps.task) {
      this.scrollView.scrollTo({
        x: Dimensions.get('window').width * nextProps.task,
      });
    }
  }
      ...
      <View>
        ...
        <Container>
          ...
          <TimerButton
            ...
            content={
              ticks ? new Date(ticks * 1000).toISOString().substr(11, 8) : null
            }
            flex={2}
          />
          <IconButton
...
const renderContent = (updateRoute, task, ticks) =>
  recipe => (
    <View>
      <Row
        height={150}
...
export default (
  { triggerDemo, data, navigation: { state: { params }, setParams } }
) => (
  <Scroll
